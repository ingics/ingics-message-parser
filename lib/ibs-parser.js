...
const ibs01FeatureMap = new Map([
    ...
    [ 0x05, { name: 'iBS01T', temp: true, humidity: true, events: [ bitButton ] } ],
...
const ibsFeatureMap = new Map([
    ...
    [ 0x02, { name: 'iBS02IR2', temp: false, humidity: false, events: [ bitIR ] } ],
    [ 0x04, { name: 'iBS02M2', temp: false, humidity: false, events: [ bitDin ] } ],
    ...
    [ 0x21, { name: 'iRS02TP', temp: true, humidity: false, events: [ bitHall ], tempExt: true } ],
    ...
    [ 0x35, { name: 'iBS05T', temp: true, humidity: false, events: [ bitButton ] } ],
    [ 0x36, { name: 'iBS05G', temp: false, humidity: false, events: [ bitButton, bitMoving, bitFall ] } ]
])
    ...
    if (feature) {
        ...
        if (feature.tempExt) {
            if (feature.temp) {
                ...
                this.temperature = this.raw.readInt16LE(9) / 100
            }
        }
        if (feature.tof) {
            this.range = this.raw.readInt16LE(9)
        }
        if (feature.events) {
...
exports.parse = function () {
    if (this.raw.length >= 4) {
        ...
        } else if (this.mfg === 0x0D && code === 0xBC85) {
            // iBS03GP
            rg.bind(this)()
        } else if (this.mfg === 0x082C && code === 0xBC85) {
            // iBS05
            ibs.bind(this)(ibsFeatureMap)
        }
    }
...
exports.supportEventList = function () {
    const events = []
    ibsEventMap.forEach((value, /*key*/) => {
        events.push(value)
    })
...
exports.supportSensorList = function () {
    return [ 'battery', 'temperature', 'temperatureExt', 'humidity', 'range', 'gp' ]
}
