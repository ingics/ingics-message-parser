...
function msd (msdBuf) {
    if (Buffer.isBuffer(msdBuf)) {
        ...
        if (obj.type.startsWith('iBS') || obj.type.startsWith('iRS')) {
            const ibs = require('./ibs-parser').igsMapping.bind(this)
            ibs(obj)
            ...
            this.tx = undefined
            if (Object.keys(obj).includes('ref_tx')) {
                this.tx = obj.ref_tx
            } else if (Object.keys(obj).includes('tx_power')) {
...
// helper function for checking device type
